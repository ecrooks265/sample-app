...
using SampleApp.Models.VMs.Account;
using SampleApp.Services.Misc;
using SampleApp.Services.Account;

namespace SampleApp.Server.Services.Middleware.CustomIdentityAPI;
/// <summary>
...
public static class IdentityApiEndpointRouteBuilderExtensions
{
        ...
        where TUser : class, new()
    {
        ...
        var emailSender = endpoints.ServiceProvider.GetRequiredService<ISendGridEmailService<TUser>>();
        ...
        routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>> (
            ...
            [FromServices] IServiceProvider sp) => {
                ...
                string requestingUrl = "None";
                if (!StringValues.IsNullOrEmpty(originUrl))
                {
                    requestingUrl = originUrl.ToString();
                }
                else if (!StringValues.IsNullOrEmpty(refererUrl))
                {
                ...
                var user = new TUser();
                Spec.CopyProperties(registration, user);
                ...
                await SendConfirmationEmailAsync(user, userManager, context, email, requestingUrl);
        ...
        routeGroup.MapPost("/resendConfirmationEmail", async Task<Ok>
            ([FromBody] ResendConfirmationEmailRequest resendRequest, HttpContext context, [FromServices] IServiceProvider sp) => {
                ...
                await SendConfirmationEmailAsync(user, userManager, context, resendRequest.Email, requestingUrl);
                return TypedResults.Ok();
            });
        ...
        routeGroup.MapPost("/forgotPassword", async Task<Results<Ok, ValidationProblem>>
            ([FromBody] ForgotPasswordRequest resetRequest, HttpContext context, [FromServices] IServiceProvider sp) => {
                ...
                if (user is not null && await userManager.IsEmailConfirmedAsync(user))
                {
                    ...
                    await emailSender.SendPasswordResetEmailAsync(user, resetRequest.Email, HtmlEncoder.Default.Encode(code), requestingUrl);
                }
        ...
        accountGroup.MapPost("/info", async Task<Results<Ok<InfoResponse>, ValidationProblem, NotFound>>
            (ClaimsPrincipal claimsPrincipal, [FromBody] InfoRequest infoRequest, HttpContext context, [FromServices] IServiceProvider sp) => {
                var userManager = sp.GetRequiredService<UserManager<TUser>>();
                var requestingUrl = context.Request.GetDisplayUrl();
                if (await userManager.GetUserAsync(claimsPrincipal) is not { } user)
                {
                    return TypedResults.NotFound();
                }
                ...
                if (!string.IsNullOrEmpty(infoRequest.NewEmail))
                {
                    ...
                    {
                        await SendConfirmationEmailAsync(user, userManager, context, infoRequest.NewEmail, requestingUrl, isChange: true);
                    }
                }
        ...
        async Task SendConfirmationEmailAsync(TUser user, UserManager<TUser> userManager, HttpContext context, string email, string requestingUrl, bool isChange = false)
        {
            ...
            await emailSender.SendConfirmationEmailAsync(user, email, HtmlEncoder.Default.Encode(confirmEmailUrl), requestingUrl);
        }
